...
{
    // Buscando as criptomoedas com paginação, 15 por página
    $criptos = Criptomoedas::select('id', 'sigla', 'nome', 'valor')->paginate(8);
    ...
    if ($criptos->count() > 0) {
        return response()->json([
            ...
            'data' => $criptos->items(), // Retorna os itens da página atual
            'total' => $criptos->total(), // Total de registros
            'current_page' => $criptos->currentPage(), // Página atual
            'last_page' => $criptos->lastPage(), // Última página
            'next_page_url' => $criptos->nextPageUrl(), // URL da próxima página
            'prev_page_url' => $criptos->previousPageUrl() // URL da página anterior
...
public function listar(Request $request)
    {
        // Paginando as criptomoedas (15 por página)
        $registros = Criptomoedas::paginate(8);
